...
Current status: the native LSP request-derivation foundation is complete before runtime wiring. The milestone added deterministic typed `derive_native_lsp_reference_request` request derivation, computes LSP UTF-16 character positions from manifest-owned source text, returns typed no-op results instead of guessing when exact identifier position cannot be proven, maps explicit endpoint positions with safe file-endpoint fallback for usage-site references, and corrects native `textDocument/references` edge direction to reference site/file -> queried symbol. No runtime sync/index spawning has been added yet. Implemented in `5ab0e2cf3` and `e964b3a2f`. Verification evidence: `cargo test -p forge_project_model producer` passed; `cargo check -p forge_project_model` passed; final critic/adversarial gate PASS. Next likely direction: add runtime wiring only through the typed derivation surface while preserving manifest-owned source mapping, typed no-op safety, and the Harvard sensor/writer boundary.
