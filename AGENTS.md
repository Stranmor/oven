...
Current status: manifest-owned Cargo metadata lexical evidence is complete in `a3d88fbaa`, with repository learning metadata compatibility remediated in `72ede06cb`. Learning ledger append idempotency is now hardened through typed `LearningLedgerAppendOutcome` reporting and replay/target-state validation fixes in `8d4be8c7d`, `2b0cd2cf4`, `601c65a1`, `f25670666`, `a8a27df75`, `c64a3a6a0`, and `544c40d37`. Cargo workspace, package, target, and dependency metadata are searchable through manifest-owned lexical documents using reserved opaque `cargo:` evidence IDs; shared evidence resolution handles symbol, shard, file, and Cargo targets; Cargo evidence resolves to the owning `Cargo.toml` for safe whole-file readback; and service readback content comes from file readback rather than lexical metadata text. `explain-context` exposes query-specific planner diagnostics derived from `forge_project_model::plan_project_context_retrieval` while staying read-only: no `query_workspace`, source range readback, context-pack write, or episode append. These bounded, redaction-safe diagnostics render separately from replay preview and landed in `0912e3064`. Pure semantic retrieval activation is now represented as a planner seam in `crates/forge_project_model`: `ProjectContextRetrievalOptions<'a>` accepts typed precomputed `VectorQuery` input and a ready vector index boundary, preserving default/no-vector fallback, provider/service/UI/config non-integration, typed vector/reranker boundaries, and redaction-safe phase diagnostics in `6c2d33fc`. Verified with focused retrieval-plan, legacy deserialization, explain-format, `explain_context`, representative `inject` tests, `cargo test -p forge_project_model retrieval_plan`, `cargo test -p forge_project_model`, `cargo check -p forge_project_model`, `cargo check -p forge_project_model -p forge_domain -p forge_app -p forge_main`, `git diff --check`, `cargo fmt --all`, and critic/adversarial PASS.
