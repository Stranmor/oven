...
Current status: SemSearch Build/Update Diagnostics v1 is active in `44ab48c04`, adding the read-only `forge workspace vector-index diagnostic [path] --porcelain` API/CLI with closed status/action taxonomy, stable reason labels, redaction-safe configured/missing model reporting, optional manifest/vector identities, structured `argv` build command plus display-only quoted command, and no provider calls, vector build/update, manifest refresh, or cleanup side effects; verified by focused `forge_services`, `forge_app`, `forge_main`, and cross-crate check suites plus `git diff --check` and critic PASS. Project Model Semantic Vector Build + CLI Query Bridge v1 is active in `c36e1e436`, `ab7e8f35f`, and `612a2a132`; SemSearch Durable Vector Runtime Bridge v1 is active in `bcec5b198`; Automatic Project-Context Semantic Injection Bridge v1 is active in `025856fed`, with stale-artifact remediation in `9393b3610` and `b543b14ee2313eee06ba4c5f3cc16e9b76b8887d`; Semantic Vector Readiness + Explain-Context Diagnostics Bridge v1 is active in `366beaa65`; SemSearch Availability + Error-Surface Diagnostics Bridge v1 is active in `1e482acfb`; and shared learning sensor sanitized observation state was preserved separately in `e4ae4e1df17828b476011da6cb39aad7f62e6497`, with later shared-state learning/conversation compile blockers forward-integrated during SemSearch test hardening. The project model owns provider-neutral workspace embedding inputs, durable vector build/readiness reports, freshness/provenance diagnostics, and context-pack integration, while production embedding execution stays behind the external typed boundary in `forge_services`, outside `forge_project_model`. `workspace vector-index build` reads a fresh manifest, builds typed embedding inputs, writes and readback-validates the durable vector artifact, and treats missing, corrupt, ambiguous, model/dimension-mismatched, empty, non-finite, or zero-norm cases as diagnostic failures rather than fallback success. `workspace query --semantic --embedding-model-id` computes the query embedding through the same boundary and feeds `SearchParams.query_embedding` plus `embedding_model_id` into the existing semantic runtime path. `sem_search` now uses typed read-only readiness for advertisement instead of only remote indexed/authenticated flags: readiness verifies semantic embedding model configuration, manifest freshness, and one matching durable vector index before normal availability; unsupported states are not advertised; `Unknown` remains explicitly available for typed diagnostic failure instead of silent disappearance; the executor repeats the same preflight before embedding provider calls so non-ready states fail before provider/query/build side effects; the cache key includes the semantic readiness fingerprint to prevent stale tool-definition availability; and there is no reranker activation or auto-build side effect. Automatic project-context injection now performs a typed durable-vector readiness preflight before embedding, calls embedding only when a matching current vector index is ready, falls back lexical without embedding when there is no model, no vector, or no current match, and emits diagnostic markers without blocking the main provider request for provider failure, timeout, dimension mismatch, or invalid vector states. Stale readable, unreadable, or corrupt nonmatching vector artifacts no longer poison readiness or runtime selection when a valid current vector index exists; corrupt current matching artifacts remain corrupt/not-ready, ambiguous current matches remain ambiguous, and indexing/build/reranker scope does not creep into the injection hot path. The tool description no longer claims active production reranking while reranker runtime remains unwired. Verified with `cargo test -p forge_app sem_search --lib`, `cargo test -p forge_services sem_search_availability --lib`, `cargo check -p forge_app -p forge_services -p forge_domain`, `git diff --check`, and external critic PASS, plus prior focused `forge_project_model`, `forge_services`, `forge_main`, workspace CLI, SemSearch runtime, injection, learning shared-state, format, clippy, critic, and adversarial gates for the predecessor milestones.
