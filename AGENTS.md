...
Current status: project-model injection correctness and the retrieval quality baseline are implemented in `e47ad48e25b202eea222dc3c8f341e701f68dcb0` and `887cdf49e0fadc77b4d134216cb1863b3d510e42`. Manifest freshness is checked against the filesystem before injection and passed into `ContextPack`; exact-path and path-prefix retrieval scopes are typed and enforced before truncation/graph expansion; lexical retrieval includes bounded shard source text; provider cache tests verify project-model context remains cache-ineligible. Verified with focused project-model/context/cache test suites, `cargo check -p forge_project_model -p forge_services -p forge_app`, `git diff --check`, and adversarial critic PASS; full `cargo insta test` is pending local `cargo-nextest` availability, with no snapshot changes in this milestone.
