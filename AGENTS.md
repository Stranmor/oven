...
Current status: the Project Representation / Model Context layer now owns the retrieval execution plan/source-readback seam. In `67a8d979a`, `forge_project_model` added a pure retrieval planner that constructs `RetrievalQuery`, path scope, freshness proof gating, stale rejection, and pack/read planning; the service layer now performs IO/readback/write-episode conversion after receiving the model-layer plan. In `25b31665df1e5dd6ad0d0978af4fb306b3861c5a`, read request path validation rejects `.` and `safe/./secret` current-directory separator tricks. Verified with focused `project_model` retrieval-plan tests, services `query_workspace` tests, `forge_app` `explain_context` tests, touched-crate check/clippy/fmt/diff-check, and final critic PASS.
