...
Current status: the minimal native-normalizer-backed LSP production seam is complete. The milestone added an explicit single-request seam with typed input (`NativeLspSourceEndpoint`, `NativeLspFileOpenPlan`, `NativeLspReferenceRequest`), a private non-persistent sensor, bounded `Content-Length` parsing, exact request-id accounting, the rust-analyzer session sequence, and writer publication only after full success. The critic-discovered JSON-RPC ordering defect is fixed: out-of-order non-error responses no longer mask later valid references. Implemented in `f0784d106edbfdd84f64470e5398a7be7a564b7c`, `1951bc7f344816749f9dccf5e6515c108f001a18`, and `f0b750cc6ae8df3f6c0ab22e76ec4fe3b27aa0c0`. Verification evidence: `cargo test -p forge_project_model producer` passed; `cargo check -p forge_project_model` passed; final critic/adversarial gate PASS. Next likely direction: extend live reference production only through the typed seam while preserving manifest-owned mapping, bounded parsing, exact id accounting, and the Harvard sensor/writer boundary.
