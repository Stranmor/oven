...
Current status: the typed LSP exact-fact producer boundary is complete as the next narrowed `forge_project_model` slice. A capability-probed producer API now exists under `forge_project_model`; producer failure returns a typed zero-fact/no-artifact report. This first slice is reference-only over already-known manifest endpoints, not a broad multi-source producer. Producers write only through `write_external_fact_artifact()` with no direct manifest mutation, no ingestion call, and no index-time spawning. No raw LSP JSON/stdout/stderr is persisted; no provider/model APIs, vector calls, or ranking/scoring changes were added. `producer_snapshot_fingerprint` was added, and the writer preserves non-empty producer `manifest_hash_input`, so stale production is rejected instead of rebound to the current manifest. Implemented in `580feb5f6` across `crates/forge_project_model/src/producer.rs`, `crates/forge_project_model/src/ingestion.rs`, `crates/forge_project_model/src/types.rs`, `crates/forge_project_model/src/util.rs`, `crates/forge_project_model/src/indexer.rs`, and `crates/forge_project_model/src/lib.rs`. Verification evidence: final critic PASS; `cargo test -p forge_project_model producer` 10 passed; `cargo test -p forge_project_model external_fact` 7 passed; `cargo test -p forge_project_model graph_expansion` 4 passed; `cargo check -p forge_project_model` passed; `cargo clippy -p forge_project_model -- -D warnings` passed; `git diff --check` passed; final report working tree was clean. Next likely direction: next reconnaissance after the typed producer boundary, likely real capability-probed live `rust-analyzer` adapter, graph/search weighting over exact facts, or vector provider connection depending on evidence.
