...
Current status: project-model injection correctness, retrieval quality baseline, durable context-pack artifact persistence, redaction-safe search episode capture, artifact/episode evaluation with knowledge-graph linkage, typed persisted external exact fact batch ingestion, and provider-neutral durable vector index artifacts are implemented. The vector artifact boundary in `crates/forge_project_model` stores only precomputed embeddings, with no provider/API/process/credential/model-router surface; artifacts carry version, manifest hash, embedding model ID, dimension, sorted entries, source identity/fingerprints, deterministic index fingerprint, and a hash-only artifact ID. Persistence uses stable JSON with temp-write/rename/directory-sync/readback validation plus thin `ProjectIndexer` list/read APIs. Validation rejects empty, duplicate, missing, stale, manifest-mismatched, dimension-mismatched, zero-norm, non-finite, overflowing, or corrupt fingerprint/ID data; search implements the existing `VectorIndex`, validates query arithmetic, prevents NaN/inf scores, preserves deterministic cosine ranking, and leaves `retrieve_with_boundaries` integration unchanged. Implemented through `e47ad48e25b202eea222dc3c8f341e701f68dcb0`, `887cdf49e0fadc77b4d134216cb1863b3d510e42`, `9138efb5a4fcd37ecb656b76aac915dae669608e`, `3f7650b004eacd1248d7aefb88e636e16e138218`, `76fe7c0e474e1e477082c053ae3caf178d483e57`, `afad02a9f2f769c1ee4b14af2a94a663d7a199e2`, `cb10812ca75b3f89959bce3488884328fd8841f9`, `1fe1ebb07d6372c5a07cdb48a5efa332d4b68963`, `33e435bd41db049761323e99774ba9a4939b15df`, `51a31103a7b6de69316ea81eeee4acb21eef2670`, `d147ee780`, and `81da4da52018ff40dbbe990f33967dc4c216cacb`. Verified with targeted regressions, `cargo test -p forge_project_model` (94 passed after vector artifact), overflow-fix target/full checks, `cargo test -p forge_main test_conversation` (25 passed where run), `cargo check -p forge_project_model -p forge_main`, `cargo check -p forge_project_model -p forge_services -p forge_app`, `git diff --check`, `cargo fmt --all -- --check`, `cargo clippy -p forge_project_model --all-targets -- -D warnings`, and critic PASS.
