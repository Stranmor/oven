...
Current status: project-model injection correctness, the retrieval quality baseline, durable context-pack artifact persistence, and redaction-safe search episode capture are implemented in `e47ad48e25b202eea222dc3c8f341e701f68dcb0`, `887cdf49e0fadc77b4d134216cb1863b3d510e42`, `9138efb5a4fcd37ecb656b76aac915dae669608e`, and `3f7650b004eacd1248d7aefb88e636e16e138218`. Manifest freshness is checked against the filesystem before injection and passed into `ContextPack`; exact-path and path-prefix retrieval scopes are typed and enforced before truncation/graph expansion; lexical retrieval includes bounded shard source text; provider cache tests verify project-model context remains cache-ineligible. Successful `WorkspaceService::query_workspace` calls persist deterministic, hash-addressed context-pack metadata artifacts only after freshness proof, retrieval, line-range conversion, and source readback into returned Nodes; stale, unknown, empty, or readback-failed paths write no successful artifact, and artifacts exclude raw source chunks, rendered prompt XML, provider DTOs, raw query/chat/tool I/O, and credentials. Successful non-empty local `query_workspace` retrievals also append `ToolEpisode` records only after node/source readback, deterministic context-pack artifact write, and artifact ID computation; episodes store only tool/status constants, timestamp, input/output fingerprints, and provenance to `context_packs/<artifact>.json`, write none on stale/unknown/empty/readback/write-failed paths, and return an error if append fails after pack write. Verified with focused project-model/context/cache/artifact/tool-episode test suites, full `cargo test -p forge_project_model`, focused `forge_services` query-workspace/context-pack suites, `cargo check -p forge_project_model -p forge_services`, `git diff --check`, and adversarial critic PASS; `cargo check -p forge_project_model -p forge_services -p forge_app` is currently blocked by unrelated dirty `forge_app` changes, and full `cargo insta test` remains pending local `cargo-nextest` availability with no snapshot changes in this milestone.
