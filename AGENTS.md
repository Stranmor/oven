...
Current status: Project Artifact Graph Activation v1 is active in `7cb4e2a754b41de787a1e973b99ea4d49e8e5f06` (`Activate project artifact graph`). `crates/forge_project_model` now models metadata-only artifacts as typed graph nodes through `ArtifactGraphNode`, `KnowledgeGraphNodeId::Artifact`, `KnowledgeGraphNodeKind::Artifact`, reserved `artifact:` namespace precedence, explicit `ArtifactDerivedFromFile` edges, artifact nodes in `KnowledgeGraph::from_manifest`, deterministic artifact-to-file indexer edges, and graph-expanded lexical retrieval from artifact seeds to linked files. Boundaries remain unchanged: artifact graph payloads store metadata only; raw config/control/prompt bodies remain out of graph storage; provider/service/UI/config runtime integration remains deferred; artifact readback remains validated. Verified with `cargo fmt --all`, `cargo test -p forge_project_model artifact`, `cargo test -p forge_project_model retrieval`, `cargo test -p forge_project_model graph`, `cargo test -p forge_project_model`, `cargo check -p forge_project_model`, `git diff --check`, and external critic/adversarial PASS.
