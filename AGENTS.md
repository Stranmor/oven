...
Current status: bounded external fact ingestion foundation is complete in `43a8eff38`, explicit opt-in exact-fact reference is complete in `56418c827`, and the read-only persisted-truth exact-fact status gate is complete in `8e07ff9ec` plus freshness remediation `79ad18bee40580076fd631b14d82e452480c5141`. `forge workspace exact-fact reference --path . [--porcelain]` runs exactly one bounded native LSP reference production attempt; it is not workspace sync. `forge workspace exact-fact status --path . [--porcelain]` is its read-only persisted-truth audit sibling: it does not spawn `rust-analyzer`, call producer/driver/probe/derivation paths, write manifests/reports/artifacts, or read raw LSP payloads. Status reports the typed current exact-fact activation state from the persisted manifest, ingestion report, artifact-store metadata, and current workspace source-hash validation. `Active` requires current workspace-tracked source hashes to match the persisted manifest, accepted external fact batches/fingerprint, and graph-visible `References + ExactCompiler` edges; self-consistent persisted JSON alone is stale when tracked source files changed or disappeared. The reference command uses frozen baseline manifest/source data, derives one typed request, probes `rust-analyzer`, produces at most one typed artifact, re-ingests from the frozen baseline, and writes the refreshed manifest/report once. Report/status output is redaction-safe, includes typed no-op outcomes, and supports porcelain JSON. Verification evidence: status-specific project_model tests, focused package services/main/app tests, affected-crate `cargo check`/`cargo clippy`, stale-active critic remediation, and final critic PASS; previous reference verification also covered `forge_project_model`, `forge_services`, and `forge_main` tests, relevant `cargo check`, `cargo clippy -D warnings` for project_model/services/main, broader domain/app/services/api/main `cargo check`, `git diff --check`, and critic PASS.
