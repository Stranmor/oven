...
Current status: bounded external fact ingestion foundation is complete in `43a8eff38`, and the explicit opt-in exact-fact reference command is complete in `56418c827`. `forge workspace exact-fact reference --path . [--porcelain]` runs exactly one bounded native LSP reference production attempt; it is not workspace sync. `workspace sync` remains pure manifest/index ingestion and must not spawn `rust-analyzer`. The command uses frozen baseline manifest/source data, derives one typed request, probes `rust-analyzer`, produces at most one typed artifact, re-ingests from the frozen baseline, and writes the refreshed manifest/report once. Report/status output is redaction-safe, includes typed no-op outcomes, and supports porcelain JSON. Verification evidence: `forge_project_model`, `forge_services`, and `forge_main` tests passed; relevant `cargo check` passed; `cargo clippy -D warnings` passed for project_model/services/main; broader `cargo check` passed for domain/app/services/api/main; `git diff --check` passed; final critic PASS.
