...
Current status: the external fact production baseline foundation is complete. The milestone added `ExternalFactProductionBaseline` as a named public DTO without a serde persistence surface, and `ProjectIndexer::external_fact_production_baseline()` now returns a no-external-facts base manifest plus transient hash-verified Rust source texts. Guards cover Rust-only manifest files, manifest-relative paths, symlink/root/path traversal rejection, UTF-8/read/hash verification, no second source walk, no artifact/report/manifest persistence, and no producer spawning. Implemented in `fccfadc7b`. Verification evidence: `cargo test -p forge_project_model indexer` passed; `cargo test -p forge_project_model producer` passed; `cargo check -p forge_project_model` passed; `cargo clippy -p forge_project_model -- -D warnings` failed only on unchanged pre-existing `producer.rs` findings classified out of scope; final critic/adversarial gate PASS.
