...
Current status: Automatic Injection Committed Query Consumption v1 is complete in `e27dc95cc5a364c4d22b7b47377a0042ca9ca8b7`. Automatic project-model context injection now consumes `WorkspaceService::query_workspace_committed(...)` internally instead of legacy `query_workspace(...)`: typed `EpisodeAppendFailed` with non-empty readback-verified nodes still injects those nodes, while real committed-query `Err` safely skips the target. Stable render label/source and public APIs/output shapes are intentionally unchanged, with no cache-stable payload churn. Tests/harness now distinguish committed vs legacy query calls, and explain-context read-only tests assert zero committed and zero legacy queries. Verification passed focused `forge_app` regressions, `cargo test -p forge_services query_workspace_committed`, `cargo test -p forge_services query_workspace_returns_error_when_episode_append_fails_after_pack_write`, `cargo check -p forge_app -p forge_services -p forge_project_model`, and final critic PASS.

Current status: Public API Query Committed Consumption v1 is complete in `67a803f55e8b61cea4c281f61579c88cffbd27b3`. `ForgeAPI::query_workspace(...)` now consumes `WorkspaceService::query_workspace_committed(...)` internally while still returning only `Vec<Node>`; the public `API::query_workspace(...)` trait shape, UI query output, and public JSON shape are unchanged. Typed `EpisodeAppendFailed` with non-empty nodes no longer hard-fails the public/manual query path, while committed hard `Err` still propagates. Regressions prove the committed path is called, the legacy path is not called, and committed metadata does not leak into public JSON output. Verification passed focused `forge_api`, `forge_services`, and `forge_app` query/project-model tests; focused check/clippy for `forge_api`, `forge_app`, `forge_services`, and `forge_project_model`; `cargo fmt --all --check`; and final critic PASS.

Current status: Committed-only Workspace Query Surface v1 is complete in `eff6231889e1f6961f5ae84c59b59e1b9574e5be`. `WorkspaceService::query_workspace(...)` remains only as the legacy `Vec<Node>` compatibility API and now defaults through `query_workspace_committed(...)`, with legacy projection centralized in `project_committed_query_nodes(...)`. Typed `EpisodeAppendFailed` no longer becomes a hard error for legacy/public node projection after a committed query succeeds with nodes, while committed hard errors still propagate before projection. Production `ForgeWorkspaceService` implements committed query only and no longer owns a separate `query_workspace(...)` / `query_local_workspace(...)` policy branch. Public API, `sem_search`, automatic injection outputs, and metadata-leak boundaries remain unchanged. Verification passed focused workspace/query/API/injection tests, package check/clippy for `forge_app`, `forge_services`, `forge_project_model`, and `forge_api`, `cargo fmt --all --check`, and final critic PASS.

Current status: Runtime Reranker Config/Readiness Selector v1 is complete in `f1fbd48a4e5891301664c4db02316e30c2450db1`. `ForgeServices::new(...)` now carries the concrete production runtime reranker selector from `crates/forge_services/src/context_engine.rs` instead of the default generic missing selector. The selector is side-effect-free and fail-closed: missing accepted runtime source remains `Missing`, configured-but-not-ready maps to `Unavailable` / `RerankerNotReady`, and `Ready` is constructible only for an already-built synchronous non-network `Reranker` boundary. No live provider/model call or fake-ready deterministic production reranker was introduced. Verification passed target runtime reranker tests, production selector tests, the `forge_app` explain-context missing-runtime test, focused check/clippy for `forge_project_model`, `forge_services`, and `forge_app` with `-D warnings`, `cargo fmt --all --check`, `git diff --check`, and external critic PASS.

Current status: Typed Rerank-Intent Handoff v1 is complete in main commit `192b84163` with remediation commit `c1209efbe50e783b4962ee75e6ca297d4e09eb72`. Project-model reranking now uses typed `RerankIntentSource` / `RerankIntent` surfaces that separate retrieval query text from rerank intent: reranker selection consumes the typed intent, while lexical/vector retrieval and `sem_search` embeddings remain based on `query`, not `use_case`. Automatic project-model injection uses a typed automatic query fallback instead of allowing a generic fixed use-case to override the actual query; replay/history identity includes rerank intent source/fingerprint to avoid same-query/different-use-case collisions; diagnostics expose source/fingerprint/len without a raw rerank payload field. Critic found and remediation fixed explicit rerank intent not activating when query text was empty but exact candidates existed. Verification passed focused `forge_project_model` rerank tests including `explicit_rerank_intent_reranks_exact_candidates_when_query_text_is_empty`, target `forge_services`/`forge_app`/`forge_main` tests, cargo check/clippy/fmt/diff checks, and external critic PASS.

Current status: Offline Rerank Artifact Config/Source Activation v1 is complete in `e218df39d` with follow-up schema commit `ce2ffc96e`. Production offline/cache rerank-score artifact activation is wired through typed config at `project_context.offline_rerank_score_artifact.path`; `ForgeServices::new(...)` resolves that config against the current workspace root and constructs the production runtime reranker selector. Semantics are fail-closed: absent config stays `Missing`; unreadable, corrupt, stale, or manifest-mismatched artifacts become redaction-safe `Unavailable` / `RerankerNotReady`; manifest-compatible artifacts become `Ready`. Exact per-query candidate-key matching remains inside the validated reranker at query time, so mismatches inject no scores and do not make construction unreadiness. The original mixed commit was preserved forward-only and workflow files were accounted as shared state. Verification passed focused config/services tests, relevant crate check/clippy/fmt/diff checks, final independent critic PASS, and clean working tree.

Current status: Durable Offline Rerank Artifact Config/Source Hardening v1 is complete. The offline rerank trusted path TOCTOU symlink replacement bypass is fixed in `cbe0a3e44aef` with final gate PASS. During preservation, the parallel `forge_quality_runtime` workspace shared-state set became coherent active workspace state and was preserved forward; quality-runtime stabilization is complete through `40c07566f4168a161adae228b7996be63338662b`, covering nested secret redaction/rejection, required-dimension not-checked handling, and evidence/artifact-hash mismatch regressions. Verification evidence: `cargo fmt --all --check`, `cargo test -p forge_quality_runtime`, `cargo check -p forge_quality_runtime -p forge_app -p forge_domain -p forge_infra -p forge_main`, targeted `forge_infra` process kill regression, independent critic PASS, and final clean git status.

Current status: Offline Rerank Runtime Availability Diagnostic Projection v1 is complete in `27b5999bf715244621e9406582916d67183ea6ff`. Explain-context diagnostics now expose a metadata-only offline rerank runtime availability snapshot that distinguishes `missing_config`, `configured_ready`, and `configured_not_ready` with stable issue labels, without constructing or calling a reranker in the read-only path. Follow-up dirty shared state was preserved and stabilized forward: quality runtime diagnostic/MCP probe state in `db3df9bee` and `0d62845a6`, generated trace ignores in `cc478fd6d`, terminal goal shared state in `f908d62a7`, and terminal goal state-transition guard repair in `9101b2841`. Verification evidence: targeted offline rerank/explain-context tests, cargo check/clippy for affected crates, `cargo check --workspace`, `cargo test -p forge_domain -p forge_app -p forge_repo`, independent critic PASS, and final clean git status.

Current status: Explain-context Runtime Rerank Applicability Projection v1 is complete in `9ca5c2b60d5b5dbd80c12664bc4052858f110405`. Explain-context now projects runtime rerank applicability from metadata-only `WorkspaceRerankRuntimeDiagnostic::project_phase_status(...)`, fixing diagnostic drift where `rerank_runtime.configured_ready=true` could coexist with `phase_diagnostics.rerank=MissingReranker` despite rerank intent. The path remains read-only: no provider calls, artifact reads, reranker calls, or scoring/fusion changes. Verification evidence: `cargo test -p forge_app explain_context_reranker`, `cargo test -p forge_domain workspace_rerank_runtime_phase_projection_skips_empty_intent`, focused `forge_services` runtime reranker tests, `cargo check -p forge_app -p forge_domain`, and external critic PASS.

Current status: Offline Rerank Artifact Applicability Diagnostics v1 is complete in `565864d99f4c6038573837f8e2b73578f7e74b10`, `66c2a434d4170691a07f4c629d966abd7bb4030b`, and `b5fc55a7664b14bb8c766b4f633e7597fb0e3a39`. Offline rerank artifacts now expose typed, redaction-safe applicability diagnostics comparing canonical artifact key with the current runtime request key; mismatch reasons are deterministic typed sets, not strings. Scoring remains exact-key fail-closed and lexical fallback behavior is unchanged. Explain-context/service diagnostics surface why a ready offline rerank artifact did not apply to a concrete request without raw query/candidate/path leaks. Verification passed `cargo test -p forge_project_model offline_rerank`, `cargo test -p forge_services offline_rerank`, `cargo test -p forge_services runtime_reranker`, `cargo test -p forge_app retrieval_plan_diagnostic_preserves_offline_rerank_applicability`, `cargo test -p forge_app explain_context_surfaces_runtime_offline_rerank_applicability`, `cargo test -p forge_main retrieval_plan_explain_format_declares_scope_and_counts`, `cargo check -p forge_project_model -p forge_services -p forge_app -p forge_domain -p forge_main`, `cargo clippy -p forge_project_model -p forge_services -p forge_app -p forge_domain -p forge_main -- -D warnings`, and external quality gate PASS. Compile-adjacent shared-state follow-up `c7532d3900e1cd6dc68a6f30c3075c192cc8569f` fixed image data URL context estimation; repository state was clean.

Current status: Durable Vector Artifact Config/Source Activation v1 is complete in `0d23a0af4` with boundary-test commit `3d809a53`. Production semantic/vector retrieval can activate a durable vector index from typed config `project_context.vector_index_artifact.path`; absent config preserves prior scan-based `.forge_project_model/vector_indexes` selection, while present config is authoritative and fail-closed with no scan fallback on rejected, missing, corrupt, stale, model/dimension-mismatched, empty, or non-finite-vector artifacts. Configured paths are canonicalized and constrained to approved local project-model artifact scope, rejecting URL-like, traversal, out-of-scope absolute, symlink, directory, and special-file paths. Valid configured artifacts pass only a validated `ProjectContextVectorIndexBoundary` through existing `ProjectContextRetrievalOptions.vector_index`; retrieval planner semantics are unchanged, and `VectorIndexArtifact::validate` remains the artifact semantic validation authority. Verification passed `cargo fmt --check`, focused `forge_services` configured/vector-artifact tests, the `forge_config` deserialization test, package check/clippy for `forge_config` and `forge_services`, final independent critic PASS, and clean working tree.
