...
Current status: the Project Representation / Model Context layer is actively advancing through read-only replay-derived diagnostics and injection-preview surfaces. The latest completed milestone is `98261772a`: `workspace explain-context` / `ProjectContextInjection::explain` is now a read-only dry-run over replay-derived, non-query-specific preview diagnostics; it no longer calls `query_workspace` or the write-capable dry-run path. Default injection behavior and `can_inject()` remain unchanged, DTO compatibility is preserved through an additive serde-default field, and branch-target listing was made read-only as a related shared-state fix. The follow-up shared-state compile blocker is resolved in `7dca725e1` through the safe `LearningService` review API and idempotency-collision regressions; the workspace was clean after post-commit `cargo check`.
