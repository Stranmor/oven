...
Current status: static Cargo workspace/package metadata is complete and verified after critic narrowing: `crates/forge_project_model` models static Cargo declarations, not Cargo resolver behavior, while preserving legacy `CargoDependency` edges. The hyphenated package/bin target type-confusion found by critic review is fixed and verified in the latest project-model commits, including `bb1cab614` and `8d268ae67`. Separate shared dirty state in `crates/forge_app/src/app.rs` was classified as parallel semantic runtime learning/chat-flow test work, integrated through forward-only commits, fixed after critic found the missing harness preload method, and verified in `58392d2b50bd31904133c338a144c03016622f7e`. Verification evidence: Cargo metadata gate PASS; app.rs gate PASS; `cargo test -p forge_app chat_flow` passed 2 tests; `cargo test -p forge_app learning_context_injection` passed 3 tests; `cargo check -p forge_app` passed; working tree was clean after the app fix. Next high-leverage direction: continue the project-model SOTA context layer after static Cargo metadata, likely with LSP/SCIP/compiler facts ingestion or graph/search integration based on the next reconnaissance.
