...
Current status: bounded external fact ingestion foundation is complete in `43a8eff38`, explicit opt-in exact-fact reference is complete in `56418c827`, the read-only persisted-truth exact-fact status gate is complete in `8e07ff9ec` plus freshness remediation `79ad18bee40580076fd631b14d82e452480c5141`, and exact-fact readiness diagnostics integration is complete in `17e634915`. Exact-fact activation/readiness is now surfaced in project-model context diagnostics and injection metadata. `WorkspaceContextManifestDiagnostic::can_inject()` remains manifest-only: inactive exact facts do not silently block manifest-only context injection. Readiness is read-only provenance plumbing over existing `read_exact_fact_status`; it does not spawn `rust-analyzer`, derive LSP, call producer/driver/probe paths, write manifests/reports/artifacts, or change default sync. `workspace explain-context` reports exact-fact readiness for selected targets and nearest skipped manifest candidates. Injected project-model context metadata includes compact exact-fact readiness/provenance without persisted `ContextPack` schema churn. Verification evidence: focused app/services/project_model tests, affected-crate `cargo check`, affected-crate `cargo clippy --all-targets -D warnings`, and final critic PASS; previous exact-fact verification also covered status/reference project_model tests, focused package services/main/app tests, broader affected-crate checks/clippy, `git diff --check`, stale-active critic remediation, and critic PASS.
