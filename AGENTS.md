...
Current status: manifest-owned Cargo metadata lexical evidence is complete in `a3d88fbaa`, with repository learning metadata compatibility remediated in `72ede06cb`. Learning ledger append idempotency is hardened through typed `LearningLedgerAppendOutcome` reporting and replay/target-state validation fixes in `8d4be8c7d`, `2b0cd2cf4`, `601c65a1`, `f25670666`, `a8a27df75`, `c64a3a6a0`, and `544c40d37`. `explain-context` exposes query-specific planner diagnostics as read-only, redaction-safe diagnostics in `0912e3064`. Pure semantic retrieval activation is represented as a planner seam in `crates/forge_project_model` with typed precomputed `VectorQuery` input, ready vector index boundary, default/no-vector fallback, typed vector/reranker boundaries, and redaction-safe phase diagnostics in `6c2d33fc`. Project Artifact Inventory v1 is metadata-only inside `crates/forge_project_model` in `dadebb828`, with empty artifact readback remediation in `77e0a8b16d7866cc2c55d9d642d6ec30deb7d7ae`: `ProjectManifest.artifacts` uses a serde default; `ProjectArtifact`, `ProjectArtifactKind`, and config format are typed; conservative deterministic classification uses already-indexed files; synthetic artifact lexical documents store metadata only; artifact evidence resolves through validated readback; empty artifact files normalize to valid whole-file readback ranges. Boundaries remain unchanged: no provider/service/UI/config runtime integration, no raw config/control/prompt body storage in artifact metadata or lexical docs, and graph nodes remain deferred. Verified with focused retrieval-plan, artifact, legacy deserialization, explain-format, `explain_context`, representative `inject` tests, `cargo test -p forge_project_model artifact`, `cargo test -p forge_project_model retrieval_plan`, `cargo test -p forge_project_model`, `cargo check -p forge_project_model`, `cargo check -p forge_project_model -p forge_domain -p forge_app -p forge_main`, `git diff --check`, `cargo fmt --all`, and critic/adversarial PASS.
