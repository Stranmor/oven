...
Current status: Project Model Semantic Vector Build + CLI Query Bridge v1 is active in `c36e1e436`, `ab7e8f35f`, and `612a2a132`; SemSearch Durable Vector Runtime Bridge v1 is active in `bcec5b198`; Automatic Project-Context Semantic Injection Bridge v1 is active in `025856fed`, with stale-artifact remediation in `9393b3610` and `b543b14ee2313eee06ba4c5f3cc16e9b76b8887d`; and shared learning sensor sanitized observation state was preserved separately in `e4ae4e1df17828b476011da6cb39aad7f62e6497`. The project model now has provider-neutral workspace embedding inputs and durable vector build reports, while production embedding execution lives behind the external typed boundary in `forge_services`, outside `forge_project_model`. `workspace vector-index build` reads a fresh manifest, builds typed embedding inputs, writes and readback-validates the durable vector artifact, and treats missing, corrupt, ambiguous, model/dimension-mismatched, empty, non-finite, or zero-norm cases as diagnostic failures rather than fallback success. `workspace query --semantic --embedding-model-id` computes the query embedding through the same boundary and feeds `SearchParams.query_embedding` plus `embedding_model_id` into the existing semantic runtime path. `sem_search` now requires configured `semantic_embedding_model_id`, calls `embed_workspace_query` once per query, passes `query_embedding` plus `embedding_model_id` to `query_workspace`, and fails fast without lexical fallback when semantic prerequisites are unavailable or invalid. Automatic project-context injection now performs a typed durable-vector readiness preflight before embedding, calls embedding only when a matching current vector index is ready, falls back lexical without embedding when there is no model, no vector, or no current match, and emits diagnostic markers without blocking the main provider request for provider failure, timeout, dimension mismatch, or invalid vector states. Stale readable, unreadable, or corrupt nonmatching vector artifacts no longer poison readiness or runtime selection when a valid current vector index exists; corrupt current matching artifacts remain corrupt/not-ready, ambiguous current matches remain ambiguous, and indexing/build/reranker scope does not creep into the injection hot path. The tool description no longer claims active production reranking while reranker runtime remains unwired. Semantic Vector Readiness + Explain-Context Diagnostics Bridge v1 is active in `366beaa65`: `workspace explain-context` now exposes read-only semantic vector readiness diagnostics separately from retrieval phase diagnostics; distinguishes durable vector readiness/config/artifact usability from query vector participation; performs no embedding provider call, `query_workspace`, writes, or reranker activation; reports reranker phase as missing/inactive runtime until production reranker exists; and keeps porcelain fields additive/defaulted. Verified with focused `forge_project_model`, `forge_services`, and `forge_main` workspace CLI tests/checks; `cargo fmt --all`; `git diff --check`; `cargo clippy -p forge_project_model -p forge_services -p forge_main -- -D warnings`; SemSearch runtime checks `cargo test -p forge_app sem_search_`, `cargo test -p forge_config`, and `cargo clippy -p forge_app --lib --tests -- -D warnings`; focused learning shared-state checks; Automatic Project-Context Semantic Injection Bridge checks `cargo test -p forge_app project_model_context_ -- --nocapture`, `cargo test -p forge_services --lib`, `cargo check -p forge_app -p forge_services -p forge_domain`, `cargo clippy -p forge_app -p forge_services -p forge_domain -- -D warnings`, additional stale-artifact regressions, and final external critic PASS; and final external critic/adversarial PASS.
