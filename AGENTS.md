...
Current status: observable exact-fact ingestion reporting and the bounded artifact writer scaffold are complete. Runtime sync now calls `ProjectIndexer::index_with_external_fact_report()` and persists a redaction-safe report at `.forge_project_model/external_fact_artifact_ingestion_report.json` instead of discarding accepted/rejected artifact state. The report includes accepted/rejected counts, artifact paths, issue codes/details, and accepted batch metadata, fingerprints, and source labels, with no raw producer/tool output. The artifact writer scaffold accepts already typed `ExternalFactBatch` values, computes deterministic source and batch fingerprints, rejects `ExternalFactSource::Unknown`, validates before write/readback, and writes content-derived artifact JSON atomically under `model_dir/external_facts`. No live LSP, SCIP, or compiler producer execution was added. Implemented in `3051fcf1cd2802b80b3c78f6d38b46ffda177eba`. Verification evidence: final critic PASS; `cargo test -p forge_project_model external_fact -- --nocapture` passed; `cargo test -p forge_project_model writer_to_indexer_ingested_edge -- --nocapture` passed; `cargo test -p forge_services sync_workspace_ -- --nocapture` passed; `cargo check -p forge_project_model -p forge_services` passed; `cargo clippy -p forge_project_model -p forge_services -- -D warnings` passed; `git diff --check` passed; final gate working tree was clean. Next likely direction: next reconnaissance after observability and writer scaffold, likely live capability-probed producer adapter, graph/search integration, or vector provider connection depending on evidence.
