...
Current status: Project Context Semantic Runtime Activation v1 is active in `1beb8fec2`, `c49a66763`, and `20b4a454d1ca100f5579279954cce4dd1dda5b16`. Runtime `query_workspace` now plans retrieval through the semantic-aware typed project-context path, while semantic vectors remain provider-neutral late-bound per-query data and provider embedding/reranker generation stays outside `forge_project_model` and the service path. Typed diagnostics preserve `use_case`, `top_k`, candidate budgets, and unavailable semantic-artifact reasons without raw source/config/control payloads. Durable vector index selection is deterministic, manifest/model/dimension validated, stale or ambiguous semantic artifacts fall back to lexical/graph retrieval, and invalid query vectors (empty, dimension mismatch, non-finite, zero norm, overflowing norm) are rejected before vector search. Verified with focused `forge_domain`, `forge_project_model`, and `forge_services` tests/checks; `cargo fmt --all`; `git diff --check`; and final external critic/adversarial PASS.
