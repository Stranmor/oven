...
Current status: durable precomputed external exact-fact artifact ingestion into the `forge_project_model` indexer is complete. The indexer consumes accepted JSON artifacts from `ProjectIndexer.model_dir/external_facts` without spawning live producers; builds the base manifest first; validates every candidate against the frozen base hash to avoid multi-artifact same-baseline mutable-hash drift; applies accepted batches deterministically; enforces an 8 MiB parse limit; reports symlink, non-JSON, and non-file rejections; recomputes fingerprints; records accepted-only provenance; and returns redaction-safe report DTOs. Model storage is excluded from source indexing for default, relative, and custom `model_dir` paths. Exact external facts now participate in retrieval graph expansion when endpoints resolve. Critic-found invalid one-based line ranges and out-of-bounds ranges against real file line counts are fixed with regressions. Implemented in `34daea8da4db2f5805602ebf59223187604c9b16`, `ad1e9285fa38f6a3a3e560af04c5e93a737c3126`, and `94bf962279215f79b8eb9a15f5492e1c04c21613`. Verification evidence: final critic PASS; targeted external/retrieval tests passed; `cargo check -p forge_project_model` passed; `cargo clippy -p forge_project_model -- -D warnings` passed; `git diff --check` passed; final gate working tree was clean. Next likely direction: next reconnaissance after exact-fact artifact ingestion, likely producer side, graph-search integration, or context-pack integration depending on evidence.
