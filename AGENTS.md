...
Current status: the bounded rust-analyzer/LSP transport, parser, and probe scaffold is complete as the next `forge_project_model` milestone with Harvard separation. The sensor/probe boundary now includes `RustAnalyzerProcess`, `LspTransport`, `RustAnalyzerCapabilityProbe`, `RustAnalyzerBounds`, `RustAnalyzerReferenceRequest`, capability/probe/report types, a bounded JSON-RPC parser, and a thin typed `RustAnalyzerReferenceProducer`. No raw LSP JSON/stdout/stderr/transcripts are persisted as model facts, reports, or artifacts. `ProjectIndexer::index()`, ingestion, retrieval, and context injection remain non-spawning; rust-analyzer is not spawned at index time. The producer writes only validated typed batches through `write_external_fact_artifact()` and returns typed non-writing reports for unavailable tools, timeouts, process failures, malformed JSON, missing semantic fields, empty results, bounds failures, and invalid endpoints. The milestone also fixed the `crates/forge_services/src/context_engine.rs` fixture metadata by filling `producer_snapshot_fingerprint`. Implemented in `c02955ee27ba6de6915da4184c6609bc517b148e`. Verification evidence: final critic PASS; `cargo test -p forge_project_model rust_analyzer` 6 passed; `cargo test -p forge_project_model producer` 22 passed; `cargo test -p forge_project_model external_fact` 7 passed; fake process unavailable/probe/timeout/malformed/missing kind/zero refs/valid refs/unknown path or endpoint/bounds/determinism/indexer non-spawning tests covered; `cargo check -p forge_project_model` passed; `cargo clippy -p forge_project_model --all-targets -- -D warnings` passed; `cargo clippy --all-targets --all-features --workspace -- -D warnings` passed; `git diff --check` passed; final tree was clean. Next likely direction: next reconnaissance after the bounded LSP scaffold, likely a live smoke/capability-probed rust-analyzer integration command, graph/search weighting after real facts, or vector provider connection based on evidence.
