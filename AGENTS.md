...
Current status: bounded external fact ingestion foundation is complete in `43a8eff38`, explicit opt-in exact-fact reference is complete in `56418c827`, the read-only persisted-truth exact-fact status gate is complete in `8e07ff9ec` plus freshness remediation `79ad18bee40580076fd631b14d82e452480c5141`, exact-fact readiness diagnostics integration is complete in `17e634915`, read-only Project-Model Evidence Readiness Diagnostics are complete in `66d7f1e39`, and the bounded read-only Evidence Ledger Activation Surface is complete in `dc461d314` plus readable-link remediation `5c67a4f42`. Evidence readiness diagnostics cover context-pack artifacts, tool episodes, and episode-to-artifact links; surface through `workspace explain-context` and compact injected context metadata alongside exact-fact readiness; and remain bounded/tolerant: missing storage is a valid zero-count state, while corrupt artifacts, malformed episode lines, and missing links become redaction-safe issue summaries instead of panics or repairs. Evidence Ledger activation operates only over existing readable context-pack artifacts and tool episodes, returns summary/proof counters plus metadata-only graph and kind counts, and does not inject full graph, source, or tool payload by default. Activation links are based only on readable context-pack artifacts; corrupt or unreadable artifacts are not counted as linked endpoints. `WorkspaceContextManifestDiagnostic::can_inject()` remains manifest-only. These surfaces do not perform sync, producer, `rust-analyzer`, provider embedding, vector write, context-pack write, episode append, repair, or persisted `ContextPack` schema changes. Verification evidence: focused evidence tests, `cargo clippy -D warnings` for `forge_project_model`, and final critic PASS; previous exact-fact/readiness verification also covered status/reference project_model tests, focused package services/main/app tests, broader affected-crate checks/clippy, `git diff --check`, stale-active critic remediation, and critic PASS.

Current status: read-only Evidence Ledger Context Replay / Selection Surface is complete in `7304f1ddc` plus hardening commit `066beaa7b1676f6830ad83016301dc3fadba554c`. The surface is bounded inside `forge_project_model`: it reads only existing readable context-pack artifacts and tool episodes, returns reference-only typed output, issue taxonomy, manifest/freshness/path/link proof metadata, and deterministic budget/audit counters. It does not run sync, indexing, `rust-analyzer`, LSP producers, provider embeddings, or vector search; does not write context packs, tool episodes, cache, or metrics; does not mutate the `ContextPack` schema; and does not inject full source, tool, or graph payloads by default. Critic-found fixes are committed for symlink artifact escape, duplicate episode link amplification, out-of-file ranges, and evidence/provenance path mismatch coverage. Verification passed with `cargo test -p forge_project_model evidence_replay` (17 passed), `cargo check -p forge_project_model`, `cargo clippy -p forge_project_model -- -D warnings`, `cargo test -p forge_project_model` (218 passed), and final critic PASS.

Current status: read-only Evidence Replay Consumption Surface is complete in `9113501de` plus redaction hardening commit `8fb7f7805`. The surface adds workspace evidence replay diagnostics over the existing `select_evidence_ledger_replay` selector, exposing typed manifest-gated replay status, selected reference metadata, redaction-safe issue summaries, and budget counters through domain/service diagnostics. It does not hydrate source/tool content, does not replace default injection, returns not-replayed states for missing/stale/unknown manifests before artifact inspection, and preserves manifest-only `can_inject()` semantics. It does not call sync, `query_workspace`, context-pack writes, episode appends, `rust-analyzer`/LSP, vectors, embeddings, producers, or metrics writes. Critic-found fixes are committed for diagnostic DTO redaction: raw `Provenance.source` and untrusted episode-derived issue paths are no longer serialized. Verification passed with `cargo test -p forge_services evidence_replay_`, `cargo test -p forge_project_model replay_`, `cargo test -p forge_app project_model_context_exact_fact_readiness_does_not_change_can_inject`, `cargo test -p forge_services`, `cargo check -p forge_project_model -p forge_services -p forge_app`, `cargo clippy -p forge_project_model -p forge_services -p forge_app -- -D warnings`, and final critic PASS.

Current status: read-only Evidence Replay Render Preview Adapter is complete in `8f12a1a42`. `forge_project_model::render_sources_from_evidence_replay(...)` converts manifest-bound `EvidenceReplayReference` selections into metadata-only `ProjectModelContextSource` preview entries. The adapter refuses manifest/report hash mismatches, rejects stale or non-fresh references and evidence_id/path mismatches, keeps `content=None`, uses stable `metadata_only_reason=evidence_replay_reference_only`, and populates content hashes only from the current manifest. Replay provenance/path label redaction helpers are centralized in `forge_project_model`, and services reuse them instead of maintaining duplicate maps. This does not replace default injection, hydrate source/tool content, call `query_workspace`, run sync/writes/LSP/vector/provider/embedding/producers, change the persisted `ContextPack` schema, or change `can_inject()`. Verification passed with `cargo test -p forge_project_model evidence_replay_adapter`, `cargo test -p forge_app project_model_context_exact_fact_readiness_does_not_change_can_inject`, `cargo check -p forge_project_model -p forge_services -p forge_app`, `cargo clippy -p forge_project_model -p forge_services -p forge_app -- -D warnings`, `cargo test -p forge_services evidence_replay --lib`, and final critic PASS.

Current status: read-only Replay Preview Diagnostic Surface is complete in `8a99b5eb6` plus redaction hardening commit `514edf761`. The surface adds an explicit read-only evidence replay preview diagnostic path whose proof chain is manifest read -> freshness gate -> `select_evidence_ledger_replay` -> `render_sources_from_evidence_replay` -> `render_project_model_context`. It uses separate typed preview status/DTO values, remains diagnostic-only, does not replace default injection, and stays metadata-only with redacted stable labels. Preview states are typed for missing, stale, unknown, empty, refused, truncated, and previewed outcomes; missing/stale/unknown manifest states do not inspect artifacts. The diagnostic does not call `query_workspace`, sync, context-pack writes, episode appends, manifest/vector/metrics writes, repair/backfill, LSP, provider, embedding, or producers. Critic-found raw filesystem error/path leakage in `not_previewed_reason` is fixed; preview diagnostics now expose stable `manifest_read_error` / `manifest_freshness_error` labels. Verification passed with `cargo test -p forge_services evidence_replay_preview -- --nocapture`, `cargo test -p forge_services evidence_replay_preview_manifest`, `cargo test -p forge_services evidence_replay_default_diagnostic_status_remains_unchanged -- --nocapture`, `cargo test -p forge_project_model evidence_replay_adapter -- --nocapture`, `cargo check -p forge_services -p forge_project_model`, `cargo check -p forge_app`, `cargo clippy -p forge_services -p forge_project_model -- -D warnings`, `cargo clippy -p forge_services -p forge_domain --all-targets -- -D warnings`, and final critic PASS.
