...
Current status: manifest-only lexical retrieval is complete in `664301be9`. `documents_from_manifest` and `LexicalIndex::from_manifest` are deterministic from `ProjectManifest` only: lexical shard construction no longer performs hidden live file reads, tokenizes `content_hash`, or indexes raw source body text. The shard lexical surface now uses manifest metadata, symbol ids, resolved `SymbolNode` metadata, a missing-symbol marker, and stable/deduped ordering; body-only lexical queries no longer succeed solely because a backing file exists. Verified with `cargo test -p forge_project_model lexical`, `cargo test -p forge_project_model retrieval`, `cargo test -p forge_project_model retrieval_plan`, `cargo clippy -p forge_project_model -- -D warnings`, and final critic PASS.
