...
Current status: the native rust-analyzer producer first safe slice is complete after the bounded scaffold. The milestone added capability smoke coverage and a pure native LSP `Location[]` normalizer with explicit manifest-owned endpoint/position mapping while keeping full live reference production intentionally unimplemented and raw LSP confined to the sensor/parser boundary. The follow-up critic fix rejects native LSP ranges outside manifest file line bounds. Implemented in `1e61f41a8` and `3a2bed67c6cf77a401ddf41d6077a90b04f963cf`. Verification evidence: `cargo test -p forge_project_model producer` passed; `cargo check -p forge_project_model` passed; final critic/adversarial gate PASS. Next likely direction: implement the next safe live reference-production slice only after preserving the manifest-owned mapping, bounds validation, and Harvard sensor/parser boundary.
