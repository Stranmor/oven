...
fn tool_status_line(status: &forge_ui_model::UiToolStatus) -> Line<'static> {
    ...
    let detail_hint = if summary.is_empty() {
        "detail in rail".to_string()
    } else {
        format!("{summary} - detail in rail")
    };
...
fn render_truncated_payload(text: &str, prefix: &'static str, color: Color) -> Vec<Line<'static>> {
    ...
    if raw_lines.len() <= DETAIL_TRUNCATION_LINES {
        return render_body_lines(text, prefix, color);
    }
    ...
    for line in raw_lines.iter().take(head) {
        lines.push(payload_line(prefix, line, color));
    }
    lines.push(Line::from(Span::styled(
        format!("{prefix}… {omitted} lines truncated …"),
        Style::default().fg(Color::DarkGray),
    )));
...
mod tests {
    ...
    fn test_command_center_renders_80x20_compact_two_column_rail() {
        ...
        assert!(actual.contains("Call id call-1"));
