...
- Agents can be resumed using the `session_id` parameter by passing the agent ID from a previous invocation. When resumed, the agent continues with its full previous context preserved. When NOT resuming, each invocation starts fresh. Provide a clear task description, but **NEVER copy-paste large conversation histories or file contents into the task description**. If the subagent needs large context (>500 words), write it to a temporary markdown file using the `write` tool and pass the file path in the task description instead.
