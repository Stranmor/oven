...
- Agents can be resumed using the `session_id` parameter by passing the `conversation_id` returned from a previous invocation. The tool result also includes a durable `task_id` for lifecycle recovery and an explicit `conversation_id`/`session_id` for compatibility resume. Resuming is allowed only for the same delegated conversation ownership lineage; unrelated parent conversations are rejected instead of silently reparented. Use `forge task list` to inspect active task sessions, `forge task list --all` to include terminal history, and `forge task show <task_id>` to recover final result/error/status metadata. When NOT resuming, each invocation starts a fresh delegated conversation and durable task record. Provide a clear task description, but **NEVER copy-paste large conversation histories or file contents into the task description**. If the subagent needs large context (>500 words), write it to a temporary markdown file using the `write` tool and pass the file path in the task description instead.
- When the agent is done, it will return a single message back to you along with both `task_id` and `conversation_id`. Use `conversation_id` as the compatibility `session_id` for follow-up resume; use `task_id` for durable lifecycle inspection, recovery, active/all listing, and final result/error audit.
