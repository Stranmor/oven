...
impl TryFrom<forge_domain::Context> for Request {
    ...
    fn try_from(request: forge_domain::Context) -> std::result::Result<Self, Self::Error> {
        ...
        let max_tokens = match &thinking {
            Some(t) => {
                let req_max = request.max_tokens.map(|v| v as u64).unwrap_or(default_max_tokens);
                if req_max <= t.budget_tokens {
                    t.budget_tokens + 1024
                } else {
                    req_max
                }
            }
            None => request.max_tokens.map(|v| v as u64).unwrap_or(default_max_tokens),
        };
