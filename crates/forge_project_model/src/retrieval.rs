...
use crate::OfflineRerankApplicability;
...
/// Retrieval results plus diagnostic-only integration observations.
#[derive(Clone, Debug, PartialEq)]
pub struct RetrievalExecution {
    /// Final selected retrieval results.
    pub results: Vec<RetrievalResult>,
    /// Redaction-safe offline rerank applicability when the reranker exposes it.
...
/// Retrieves project model results with optional vector and typed reranker diagnostics.
///
/// # Arguments
///
/// * `manifest` - Indexed project manifest.
/// * `query` - Retrieval query.
/// * `vector_query` - Optional provider-neutral embedding generated by an external integration.
/// * `vector_index` - Optional vector index implementation.
/// * `reranker` - Optional reranker implementation.
/// * `rerank_intent` - Optional normalized intent selected independently from query text.
/// * `weights` - Typed scoring weights used by the planner.
pub fn retrieve_with_boundaries_and_rerank_intent_diagnostics<V, R>(
    manifest: &ProjectManifest,
    query: &RetrievalQuery,
    vector_query: Option<&VectorQuery>,
    vector_index: Option<&V>,
...
where
    ...
    R: Reranker + ?Sized,
{
    retrieve_with_boundaries_and_rerank_intent_inner(
        manifest,
        query,
        vector_query,
        vector_index,
...
fn retrieve_with_boundaries_and_rerank_intent_inner<V, R>(
    manifest: &ProjectManifest,
    query: &RetrievalQuery,
    vector_query: Option<&VectorQuery>,
    vector_index: Option<&V>,
    ...
    R: Reranker + ?Sized,
{
    if weights.validate().is_err() {
        return RetrievalExecution { results: Vec::new(), offline_rerank_applicability: None };
    }
    ...
    let mut offline_rerank_applicability = None;
    if plan.rerank
        && let (Some(rerank_intent), Some(reranker)) = (rerank_intent, reranker)
    {
        ...
        offline_rerank_applicability =
            reranker.offline_applicability(&rerank_intent.text, &candidates);
    ...
    RetrievalExecution { results: values, offline_rerank_applicability }
}
